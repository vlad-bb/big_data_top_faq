...
7. Detect the base branch: run `git remote show origin | grep 'HEAD branch'` to find the default branch (usually main).
8. Push the current branch to origin. If the branch has no upstream yet, use `git push -u origin HEAD`.
9. Create a PR with `gh pr create` targeting the base branch detected in step 7. Write a short PR body with: a summary bullet list of what changed and why, and a test plan.
